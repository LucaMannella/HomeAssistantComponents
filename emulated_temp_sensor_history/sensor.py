...
DEFAULT_NAME = "Emulated Temperature Sensor History"
...
""" To work properly this integration needs to have a configured "scan_interval".
    ...
    sensor:
    - platform: emulated_temp_sensor_history
      scan_interval: 300
"""
...
def setup_platform(
    ...
    if (MIN_TEMP_KEY in config) and (MAX_TEMP_KEY in config):
        add_entities(
            [
                EmulatedTempSensorHistory(
                    name, config[MIN_TEMP_KEY], config[MAX_TEMP_KEY]
                )
            ]
        )
    elif MIN_TEMP_KEY in config:
        add_entities([EmulatedTempSensorHistory(name, min_temp=config[MIN_TEMP_KEY])])
    elif MAX_TEMP_KEY in config:
        add_entities([EmulatedTempSensorHistory(name, max_temp=config[MAX_TEMP_KEY])])
    else:
        add_entities([EmulatedTempSensorHistory(name)])


class EmulatedTempSensorHistory(SensorEntity):
    ...
    def __init__(
        ...
        self._file_path = (
            "./custom_components/emulated_temp_sensor_avg/temperatures.txt"
        )
